...
| v0.3.3 | 2026-08-31 | **修复全端 in-place 升级阻断**：`windows-process-boundary.ps1`（PS 边界）此前不识别多 profile 生命周期的 `--profile=` 尾参（JS 边界已适配、PS 侧漏改），致 all 模式在「同路径活跃旧生命周期」上 `PrepareToInstall` 的 `Stop-VerifiedWorkDaddyLifecycle` 判定「Node 入口脚本不匹配」而 fail-closed，标准版无法升级运行中安装。修复：`Test-ExactNodeEntryCommandLine` 新增可选 `-ExpectedProfile`（给出时要求命令行以 `--profile=<期望值>` 收尾，未给保持旧严格行为），并贯穿 `Assert-NodeProcessIdentity`/`Get-VerifiedNodeProcessesForScript`/`Get-UniqueNodeProcessForScript`/`Stop-VerifiedWorkDaddyLifecycle`/`Assert-DaemonStatusIdentity`；`prepare-win-install.ps1`（all 分支按 target 传 `-ExpectedProfile`，并修 catch 兜底引用未定义 `dataDir/uiPort` 的报错）、`install-win.ps1`/`uninstall-win.ps1`/`apply-update.ps1` 同步传 profile。回归在 `test/windows-powershell-boundary.test.js`（动态 PS 断言 + 静态护栏）。 |
